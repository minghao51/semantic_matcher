"""
Pattern-based novelty detection strategy.
...
class PatternBasedNoveltyStrategy:
    """
    ...
    def __init__(self, known_entities: List[str]):
        """
        Initialize the pattern-based strategy with known entities.

        Args:
            known_entities: List of known entity strings to extract patterns from
        """
    ...
    def _extract_patterns(self) -> Dict[str, Any]:
        """Extract various patterns from known entities."""
        patterns = {
    ...
    def score_novelty(self, entity: str) -> float:
        """
        Score the novelty of an entity based on pattern matching.

        Returns a score between 0 (not novel, matches patterns) and 1 (novel, violates patterns).

        Args:
            entity: The entity string to score

        Returns:
            Novelty score between 0 and 1
        """
        if not entity:
            return 1.0  # Empty string is novel
        ...
        if entity_cap in self.patterns["capitalization"]:
            ...
            # Check if it's at least a common pattern
        ...
        if scores:
            ...
            # Adjust weights if we have fewer scores
    ...
    def _get_char_ngrams(self, entities: List[str], n: int = 3) -> Set[str]:
        """Extract character n-grams from entities."""
        ngrams = set()
    ...
    def _has_numbers(self, entities: List[str]) -> float:
        """Calculate fraction of entities containing numbers."""
    ...
    def _get_capitalization_patterns(self, entities: List[str]) -> Set[str]:
        """Extract capitalization patterns from entities."""
        patterns = set()
        for entity in entities:
            if not entity:
                continue
    ...
    def _get_prefix_suffix_distribution(
        ...
        """Get distribution of prefixes or suffixes."""
        counter = Counter()
        for entity in entities:
            entity = entity.strip()
            if len(entity) >= n:
...
def score_batch_novelty(
    entities: List[str], strategy: PatternBasedNoveltyStrategy
) -> List[float]:
    """
    Score novelty for a batch of entities.

    Args:
        entities: List of entity strings to score
        strategy: PatternBasedNoveltyStrategy instance

    Returns:
        List of novelty scores (0-1)
    """
    return [strategy.score_novelty(entity) for entity in entities]
